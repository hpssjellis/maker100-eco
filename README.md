...
| &#9744;  <a name="0" href="0">`a45`</a>  Lidar Basics   |  [seeed-lidar-serial.ino](seeed-sketches/seeed-lidar-serial.ino)   IMPORTANT: on Lidar GND is the yellow wire! Needs 5V to run. Connect green wire to XIAO-esp32S3 RX (D7) to receive data.         |     | Try the first code to see if the lidar works over the serial monitor, then the second code:  Power the Lidar seperately from the XIAO for safety sake until someone says you don't need to   |
